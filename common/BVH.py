...
def load(filename, start=None, end=None, order=None, world=False):
    ...
    f.close()
    rotations = Quaternions.from_euler(np.radians(rotations), order=order[::-1], world=world)
    return Animation(rotations, positions, orients, offsets, parents), names, frametime, order


def save(filename, anim, eulers, names=None, frametime=1.0 / 24.0, order='zyx', positions=False):
    """
            ...
            for j in range(anim.shape[1]):
                ...
                if positions or j == 0:
                    ...
                    f.write("%f %f %f %f %f %f " % (
                        ...
                        rots[i, j, ordermap[order[2]]], rots[i, j, ordermap[order[1]]], rots[i, j, ordermap[order[0]]]))
